      ...
      *> position variable. At the start, it's set to one (start of
      *> string) and then auto incremented by the unstring command each
      *> time it is called. It's value is the position in the source
      *> string where it "left off".
      ...
      *> destination (into) variables supplied.
           display spaces
           display "================================================="
      ...
      *> split the string up.
           ...
           move "A E%FG!HIJ|KL!MN>OP#QR!ST" to ws-source-str
           ...
           perform until ws-pointer > function length(ws-source-str)
               ...
               display "TOTAL FIELDS FILLED: "
                   ws-single-fields-filled
               end-display
               display
                   "-------------------------------------------"
               end-display
      ...
      *> destination strings.
           ...
           move "A EFG!HIJ|KLMN>O" to ws-source-str
           ...
           perform varying ws-multi-idx
               ...
               display "DELIMITER: "
                   ws-multi-delimiter(ws-multi-idx)
               end-display
               display "CHAR COUNT:"
                   ws-multi-char-count(ws-multi-idx)
               end-display
               display
                   "-------------------------------------------"
               end-display
